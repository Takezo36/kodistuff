...
class Provider:
  ...
  def showComments(self, videoId, page=None):
    ...
    commentsDialog = CommentsDialog("plugin-extrabuttons-comments.xml", xbmcaddon.Addon("plugin.program.extrabuttons").getAddonInfo('path'), "default", "1080i", comments=comments, loadMoreFunction=self.getComments, replyFunction=self.replyToComment)
